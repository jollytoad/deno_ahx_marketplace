...
export type OAuth2Client = Parameters<typeof signIn>[1] & { provider: string };

type Providers = typeof providers;
type ProviderFn = Providers[keyof Providers];
type CreateClientFn = (
  config: Parameters<ProviderFn>[0],
) => ReturnType<ProviderFn> & { provider: string };
  ...
  for (const [name, fn] of Object.entries(providers)) {
    if (typeof fn === "function") {
      if (name.toLowerCase() === providerFnName) {
        return ((config: any) => {
          const client = fn(config);
          (client as any).provider = provider.toLowerCase();
          return client;
        }) as CreateClientFn;
      }
    }
